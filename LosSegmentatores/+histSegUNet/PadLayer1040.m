classdef PadLayer1040 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = PadLayer1040(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'onnx__Concat_135'};
            this.ONNXParams = onnxParams;
        end
        
        function [onnx__Concat_135] = predict(this, onnx__Pad_134)
            if isdlarray(onnx__Pad_134)
                onnx__Pad_134 = stripdims(onnx__Pad_134);
            end
            onnx__Pad_134NumDims = 4;
            onnxParams = this.ONNXParams;
            [onnx__Concat_135, onnx__Concat_135NumDims] = PadFcn(onnx__Pad_134, onnx__Pad_134NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {onnx__Concat_135}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'PadLayer1040');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'PadLayer1040'));
            end
            onnx__Concat_135 = dlarray(single(onnx__Concat_135), 'SSCB');
            if ~coder.target('MATLAB')
                onnx__Concat_135 = extractdata(onnx__Concat_135);
            end
        end
        
        function [onnx__Concat_135] = forward(this, onnx__Pad_134)
            if isdlarray(onnx__Pad_134)
                onnx__Pad_134 = stripdims(onnx__Pad_134);
            end
            onnx__Pad_134NumDims = 4;
            onnxParams = this.ONNXParams;
            [onnx__Concat_135, onnx__Concat_135NumDims] = PadFcn(onnx__Pad_134, onnx__Pad_134NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {onnx__Concat_135}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'PadLayer1040');
...
function [onnx__Concat_135, onnx__Concat_135NumDims, state] = PadFcn(onnx__Pad_134, onnx__Pad_134NumDims, params, varargin)
...
%
% [ONNX__CONCAT_135] = PadFcn(ONNX__PAD_134, PARAMS)
%			- Evaluates the imported ONNX network PADFCN with input(s)
%			ONNX__PAD_134 and the imported network parameters in PARAMS. Returns
%			network output(s) in ONNX__CONCAT_135.
%
% [ONNX__CONCAT_135, STATE] = PadFcn(ONNX__PAD_134, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = PadFcn(ONNX__PAD_134, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
%				  ONNX__PAD_134:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
%				  ONNX__CONCAT_135:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
[onnx__Pad_134, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Pad_134, params, varargin{:});
% Put all variables into a single struct to implement dynamic scoping:
[Vars, NumDims] = packageVariables(params, {'onnx__Pad_134'}, {onnx__Pad_134}, [onnx__Pad_134NumDims]);
% Call the top-level graph function:
[onnx__Concat_135, onnx__Concat_135NumDims, state] = PadGraph1036(onnx__Pad_134, NumDims.onnx__Pad_134, Vars, NumDims, Training, params.State);
% Postprocess the output data
[onnx__Concat_135] = postprocessOutput(onnx__Concat_135, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [onnx__Concat_135, onnx__Concat_135NumDims1039, state] = PadGraph1036(onnx__Pad_134, onnx__Pad_134NumDims1038, Vars, NumDims, Training, state)
% Function implementing the graph 'PadGraph1036'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.onnx__Pad_134 = onnx__Pad_134;
NumDims.onnx__Pad_134 = onnx__Pad_134NumDims1038;

% Execute the operators:
% Pad:
[Vars.onnx__Concat_135, NumDims.onnx__Concat_135] = onnxPad(Vars.onnx__Pad_134, Vars.PadPadding1037, 0.000000, 'constant', NumDims.onnx__Pad_134);

% Set graph output arguments from Vars and NumDims:
onnx__Concat_135 = Vars.onnx__Concat_135;
onnx__Concat_135NumDims1039 = NumDims.onnx__Concat_135;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Pad_134, numDataOutputs, params, varargin)
% Function to validate inputs to PadFcn:
p = inputParser;
isValidArrayInput = @(x)isnumeric(x) || isstring(x);
isValidONNXParameters = @(x)isa(x, 'ONNXParameters');
addRequired(p, 'onnx__Pad_134', isValidArrayInput);
addRequired(p, 'params', isValidONNXParameters);
addParameter(p, 'InputDataPermutation', 'auto');
addParameter(p, 'OutputDataPermutation', 'auto');
addParameter(p, 'Training', false);
parse(p, onnx__Pad_134, params, varargin{:});
...
function [onnx__Pad_134, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Pad_134, params, varargin)
% Parse input arguments
[inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Pad_134, 1, params, varargin{:});
anyDlarrayInputs = any(cellfun(@(x)isa(x, 'dlarray'), {onnx__Pad_134}));
% Make the input variables into unlabelled dlarrays:
onnx__Pad_134 = makeUnlabeledDlarray(onnx__Pad_134);
% Permute inputs if requested:
onnx__Pad_134 = permuteInputVar(onnx__Pad_134, inputDataPerms{1}, 4);
end

function [onnx__Concat_135] = postprocessOutput(onnx__Concat_135, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
if ~anyDlarrayInputs && ~Training
    if isdlarray(onnx__Concat_135)
        onnx__Concat_135 = extractdata(onnx__Concat_135);
    end
end
% Permute outputs if requested:
onnx__Concat_135 = permuteOutputVar(onnx__Concat_135, outputDataPerms{1}, 4);
end
