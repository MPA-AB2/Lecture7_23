classdef PadLayer1050 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = PadLayer1050(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'onnx__Concat_153'};
            this.ONNXParams = onnxParams;
        end
        
        function [onnx__Concat_153] = predict(this, onnx__Pad_152)
            if isdlarray(onnx__Pad_152)
                onnx__Pad_152 = stripdims(onnx__Pad_152);
            end
            onnx__Pad_152NumDims = 4;
            onnxParams = this.ONNXParams;
            [onnx__Concat_153, onnx__Concat_153NumDims] = PadFcn(onnx__Pad_152, onnx__Pad_152NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {onnx__Concat_153}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'PadLayer1050');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'PadLayer1050'));
            end
            onnx__Concat_153 = dlarray(single(onnx__Concat_153), 'SSCB');
            if ~coder.target('MATLAB')
                onnx__Concat_153 = extractdata(onnx__Concat_153);
            end
        end
        
        function [onnx__Concat_153] = forward(this, onnx__Pad_152)
            if isdlarray(onnx__Pad_152)
                onnx__Pad_152 = stripdims(onnx__Pad_152);
            end
            onnx__Pad_152NumDims = 4;
            onnxParams = this.ONNXParams;
            [onnx__Concat_153, onnx__Concat_153NumDims] = PadFcn(onnx__Pad_152, onnx__Pad_152NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[4 3 1 2], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)isempty(A)||~isnumeric(A), {onnx__Concat_153}))
                fprintf('Runtime error in network. The custom layer ''%s'' output an empty or non-numeric value.\n', 'PadLayer1050');
...
function [onnx__Concat_153, onnx__Concat_153NumDims, state] = PadFcn(onnx__Pad_152, onnx__Pad_152NumDims, params, varargin)
...
%
% [ONNX__CONCAT_153] = PadFcn(ONNX__PAD_152, PARAMS)
%			- Evaluates the imported ONNX network PADFCN with input(s)
%			ONNX__PAD_152 and the imported network parameters in PARAMS. Returns
%			network output(s) in ONNX__CONCAT_153.
%
% [ONNX__CONCAT_153, STATE] = PadFcn(ONNX__PAD_152, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = PadFcn(ONNX__PAD_152, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
%				  ONNX__PAD_152:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
%				  ONNX__CONCAT_153:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
[onnx__Pad_152, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Pad_152, params, varargin{:});
% Put all variables into a single struct to implement dynamic scoping:
[Vars, NumDims] = packageVariables(params, {'onnx__Pad_152'}, {onnx__Pad_152}, [onnx__Pad_152NumDims]);
% Call the top-level graph function:
[onnx__Concat_153, onnx__Concat_153NumDims, state] = PadGraph1046(onnx__Pad_152, NumDims.onnx__Pad_152, Vars, NumDims, Training, params.State);
% Postprocess the output data
[onnx__Concat_153] = postprocessOutput(onnx__Concat_153, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [onnx__Concat_153, onnx__Concat_153NumDims1049, state] = PadGraph1046(onnx__Pad_152, onnx__Pad_152NumDims1048, Vars, NumDims, Training, state)
% Function implementing the graph 'PadGraph1046'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.onnx__Pad_152 = onnx__Pad_152;
NumDims.onnx__Pad_152 = onnx__Pad_152NumDims1048;

% Execute the operators:
% Pad:
[Vars.onnx__Concat_153, NumDims.onnx__Concat_153] = onnxPad(Vars.onnx__Pad_152, Vars.PadPadding1047, 0.000000, 'constant', NumDims.onnx__Pad_152);

% Set graph output arguments from Vars and NumDims:
onnx__Concat_153 = Vars.onnx__Concat_153;
onnx__Concat_153NumDims1049 = NumDims.onnx__Concat_153;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Pad_152, numDataOutputs, params, varargin)
% Function to validate inputs to PadFcn:
p = inputParser;
isValidArrayInput = @(x)isnumeric(x) || isstring(x);
isValidONNXParameters = @(x)isa(x, 'ONNXParameters');
addRequired(p, 'onnx__Pad_152', isValidArrayInput);
addRequired(p, 'params', isValidONNXParameters);
addParameter(p, 'InputDataPermutation', 'auto');
addParameter(p, 'OutputDataPermutation', 'auto');
addParameter(p, 'Training', false);
parse(p, onnx__Pad_152, params, varargin{:});
...
function [onnx__Pad_152, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Pad_152, params, varargin)
% Parse input arguments
[inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Pad_152, 1, params, varargin{:});
anyDlarrayInputs = any(cellfun(@(x)isa(x, 'dlarray'), {onnx__Pad_152}));
% Make the input variables into unlabelled dlarrays:
onnx__Pad_152 = makeUnlabeledDlarray(onnx__Pad_152);
% Permute inputs if requested:
onnx__Pad_152 = permuteInputVar(onnx__Pad_152, inputDataPerms{1}, 4);
end

function [onnx__Concat_153] = postprocessOutput(onnx__Concat_153, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
if ~anyDlarrayInputs && ~Training
    if isdlarray(onnx__Concat_153)
        onnx__Concat_153 = extractdata(onnx__Concat_153);
    end
end
% Permute outputs if requested:
onnx__Concat_153 = permuteOutputVar(onnx__Concat_153, outputDataPerms{1}, 4);
end
