...
segmented_mask_names = dir([segmented_path '\mask*.png']);
dice_scores.details = [];
images_names = {};
values = [0,1,2,3,4];
catnames = {'background', 'tumour', 'stroma', 'necrosis', 'fat'};
for num_mask = 1:length(segmented_mask_names)
    images_names{num_mask} = segmented_mask_names(num_mask).name(end-5:end-4);
    gt = categorical(imread([gt_path '\' segmented_mask_names(num_mask).name]), values, catnames);
    segmented = categorical(imread([segmented_path '\' segmented_mask_names(num_mask).name]), values, catnames);
